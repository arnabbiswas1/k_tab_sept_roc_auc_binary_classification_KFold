...
if __name__ == "__main__":
    ...
    TARGET = "claim"
    MISSING_VALUE = -999999999999999

    target = train_df[TARGET]

    combined_df = pd.concat([train_df.drop([TARGET], axis=1), test_df])
    combined_df = combined_df.fillna(MISSING_VALUE)
    ...
    train_df.to_parquet(
        f"{constants.PROCESSED_DATA_DIR}/train_processed_fillna.parquet", index=True
    )
    test_df.to_parquet(
        f"{constants.PROCESSED_DATA_DIR}/test_processed_fillna.parquet", index=True
    )
    sample_submission_df.to_parquet(
        f"{constants.PROCESSED_DATA_DIR}/sub_processed_fillna.parquet", index=True
    )
