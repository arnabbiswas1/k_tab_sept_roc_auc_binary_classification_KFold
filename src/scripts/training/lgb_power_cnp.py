"""
LGB, KFold-5, not filled, power features (cnp)
"""
...
SEED = 42
EXP_DETAILS = "LGB, KFold-5, not filled, power features (cnp)"
...
logger.info("Adding features related to power (cnp)")
features_df = pd.read_parquet(f"{constants.FEATURES_DATA_DIR}/features_power_2.parquet")
cnp_features = [name for name in features_df.columns if "_cnp" in name]
features_df = features_df[cnp_features]
