"""
LGB, KFold-5, not filled, square features
"""
...
SEED = 42
EXP_DETAILS = "LGB, KFold-5, not filled, square features"
...
logger.info("Adding features related to power (square)")
features_df = pd.read_parquet(f"{constants.FEATURES_DATA_DIR}/features_power_1.parquet")
square_features = [name for name in features_df.columns if "_square" in name]
features_df = features_df[square_features]
