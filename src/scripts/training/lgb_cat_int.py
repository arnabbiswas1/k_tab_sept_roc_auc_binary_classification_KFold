"""
LGB, KFold-5, not filled, cat interaction
"""
...
SEED = 42
EXP_DETAILS = "LGB, KFold-5, not filled, cat interaction"
...
logger.info("Adding features related to cat interaction")
features_df = pd.read_parquet(f"{constants.FEATURES_DATA_DIR}/features_fenc_cat_intrc.parquet")
freq_enc_features = [name for name in features_df.columns if "freq" not in name]
features_df = features_df[freq_enc_features]

logger.info("Generating Label Encoding")
for name in features_df.select_dtypes("object")[0:2]:
    lb = LabelEncoder()
    features_df[name] = lb.fit_transform(features_df[name])
