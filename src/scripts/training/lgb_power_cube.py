"""
LGB, KFold-5, not filled, cube features
"""
...
SEED = 42
EXP_DETAILS = "LGB, KFold-5, not filled, cube features"
...
logger.info("Adding features related to power (cube)")
features_df = pd.read_parquet(f"{constants.FEATURES_DATA_DIR}/features_power_1.parquet")
cube_features = [name for name in features_df.columns if "_cube" in name]
features_df = features_df[cube_features]
