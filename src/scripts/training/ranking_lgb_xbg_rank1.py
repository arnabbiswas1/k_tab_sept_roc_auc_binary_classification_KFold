"""
Ranking
LGB1 : LGB,K10,non-null,mean,sum,max,params/features replicated from K
LGB2 : LGB,K10, mean,sum,max,params/features/imputer/median scaler replicated from K
XGB:  missing sum, mean, sum, max, grouped imputer, Standard scaler (k=10) - Kaggle Kernel v7
"""
...
SEED = 42
EXP_DETAILS = "Rank_1: XGB(impute,SS), LGB(null), LGB(impute/scaler)"
IS_TEST = False
...
# LGB, K10,non-null,mean,sum,max,params/features replicated from K
sub_1_predition_name = (
    "sub_lgb_K10_nonull_full_data_mean_sum_max_bin_K1_params_K2_0920_1024_0.81608.gz"
)
sub_1_oof_name = (
    "oof_lgb_K10_nonull_full_data_mean_sum_max_bin_K1_params_K2_0920_1024_0.81608.csv"
)
...
# LGB,K10, mean,sum,max,params/features/imputer/median scaler replicated from K
sub_2_predition_name = (
    "sub_lgb_K10_nonull_full_data_mean_sum_max_bin_K1_params_K2_0920_1225_0.81607.gz"
)
sub_2_oof_name = (
    "oof_lgb_K10_nonull_full_data_mean_sum_max_bin_K1_params_K2_0920_1225_0.81607.csv"
)
...
# missing sum, mean, sum, max, grouped imputer, Standard scaler (k=10) - Kaggle Kernel v7
sub_3_predition_name = "predictions_xgboof_xgb_no_null_mean_sum_max_group_imputer_StandardScaler_v7_0.81639.npy"
sub_3_oof_name = (
    "oof_xgb_no_null_mean_sum_max_group_imputer_StandardScaler_v7_0.81639.npy"
)
sub_3_oof_pred = np.load(f"{constants.KAGGLE_SUBMISSION_DIR}/{sub_3_oof_name}")
sub_3_oof_pred = pd.DataFrame(sub_3_oof_pred)
sub_3_oof_pred.index = train_df.index


sub_3_test_pred = np.load(f"{constants.KAGGLE_SUBMISSION_DIR}/{sub_3_predition_name}")
sub_3_test_pred = pd.DataFrame(sub_3_test_pred)
sub_3_test_pred.index = test_df.index

oof_dfs = [sub_1_oof_pred, sub_2_oof_pred, sub_3_oof_pred]
l1_train_df = reduce(
    lambda left, right: pd.merge(left, right, on=["id"], how="left"), oof_dfs
)
l1_train_df.columns = ["id", "sub_1", "sub_2", "sub_3"]
l1_train_df = l1_train_df.set_index("id")

prediction_dfs = [sub_1_test_pred, sub_2_test_pred, sub_3_test_pred]
l1_test_df = reduce(
    lambda left, right: pd.merge(left, right, on=["id"], how="left"), prediction_dfs
)
l1_test_df.columns = ["id", "sub_1", "sub_2", "sub_3"]
...
common.update_tracking(RUN_ID, "oof_score_", oof_score, is_integer=False, no_of_digits=6)
