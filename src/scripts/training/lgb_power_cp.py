"""
LGB, KFold-5, not filled, power features (cp)
"""
...
SEED = 42
EXP_DETAILS = "LGB, KFold-5, not filled, power features (cp)"
...
logger.info("Adding features related to power (cp)")
features_df = pd.read_parquet(f"{constants.FEATURES_DATA_DIR}/features_power_2.parquet")
cp_features = [name for name in features_df.columns if "_cp" in name]
features_df = features_df[cp_features]
