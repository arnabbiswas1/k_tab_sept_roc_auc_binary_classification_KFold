"""
LGB,K10,non-null,mean,sum,max,params/features/imputers/scaler replicated from K
params & features: https://www.kaggle.com/maximkazantsev/tps-09-21-eda-lightgbm-with-folds
"""
...
SEED = 42
EXP_DETAILS = "LGB,K10,non-null,mean,sum,max,params/features/imputer/scaler replicated from K"
...
for col in test_df.columns:
    combined_df[col] = imputer.fit_transform(np.array(combined_df[col]).reshape(-1, 1))

# New features indicating if a value is near zero or not
combined_df["f5_zero"] = (combined_df["f5"] < 0.02).astype(int)
combined_df["f50_zero"] = (combined_df["f50"] < 0.02).astype(int)

# Scaling all values
s_scaler = StandardScaler()
for col in combined_df.columns:
    combined_df[col] = s_scaler.fit_transform(np.array(combined_df[col]).reshape(-1, 1))
