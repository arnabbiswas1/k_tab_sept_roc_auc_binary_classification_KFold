"""
LGB, KFold-5, not filled, fourth features
"""
...
SEED = 42
EXP_DETAILS = "LGB, KFold-5, not filled, fourth features"
...
logger.info("Adding features related to power (fourth)")
features_df = pd.read_parquet(f"{constants.FEATURES_DATA_DIR}/features_power_1.parquet")
fourth_features = [name for name in features_df.columns if "_fourth" in name]
features_df = features_df[fourth_features]
