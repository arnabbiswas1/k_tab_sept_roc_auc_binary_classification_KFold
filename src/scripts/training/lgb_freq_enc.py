"""
LGB, KFold-5, not filled, freq enc
"""
...
SEED = 42
EXP_DETAILS = "LGB, KFold-5, not filled, freq enc"
...
logger.info("Adding features related to frequency encoding")
features_df = pd.read_parquet(f"{constants.FEATURES_DATA_DIR}/features_fenc_cat_intrc.parquet")
freq_enc_features = [name for name in features_df.columns if "freq" in name]
